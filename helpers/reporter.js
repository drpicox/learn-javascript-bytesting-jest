  ...
  onRunComplete(contexts, results) {
    ...
    function haveHints(testResult) {
      return (
        ...
        testResult.failureMessages.some((fM) => fM.includes("hint"))
      );
    }
    ...
    function getFormattedTitle(testResults) {
      return chalk.bold.red(
        "  ● " + [...testResults.ancestorTitles, testResults.title].join(" > ")
      );
    }
    ...
    function getFormattedTotals() {
      ...
      if (results.numFailedTests > failedTestsShown) {
        ...
        output += chalk.bold.yellow(
          `${results.numFailedTests - failedTestsShown} failed tests hidden`
        );
        output += `, 1 failed test shown.`;
      }

      output += "\n" + chalk.bold("Test Suites : ");
      if (results.numFailedTestSuites)
        output += chalk.bold.red(`${results.numFailedTestSuites} failed, `);
      if (results.numPassedTestSuites)
        output += chalk.bold.green(`${results.numPassedTestSuites} passed, `);
      if (results.numPendingTestSuites)
        output += chalk.bold.yellow(
          `${results.numPendingTestSuites} pending, `
        );
      output += `${results.numTotalTestSuites} total`;

      output += "\n" + chalk.bold("Test        : ");
      if (results.numFailedTests)
        output += chalk.bold.red(`${results.numFailedTests} failed, `);
      if (results.numPassedTests)
        output += chalk.bold.green(`${results.numPassedTests} passed, `);
      if (results.numPendingTests)
        output += chalk.bold.yellow(`${results.numPendingTests} pending, `);
